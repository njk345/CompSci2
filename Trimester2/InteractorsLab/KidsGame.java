...
public class KidsGame extends GraphicsProgram
{
    private RandomGenerator rand = RandomGenerator.getInstance();
    private int numEggs = rand.nextInt(10,20);
    ...
    void spawnEggs () {
        ...
        for (int i = 0; i < eggs.length; i++) {
            ...
            eggs[i].scale(0.05);
            assignSpot(eggs[i],true,eggs[i].getWidth(),scrWidth-eggs[i].getWidth(),15,scrHeight-eggs[i].getHeight());
            add(eggs[i]);
    ...
    public void keyPressed (KeyEvent e) {
        if (!gameOver) {
            ...
            System.out.println(inWater(bunny));
            double scrWidth = getWidth();
            double scrHeight = getHeight();
