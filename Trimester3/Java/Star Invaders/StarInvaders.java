 ...
 * Idea of Game:
 * 
 * User plays as an X-Wing Fighter (from Star Wars) and attempts to fight off waves of Tie Fighters of different
 * shapes, sizes, and damaging capabilities, and asteroids.
 * 
 * The user can shoot the oncoming objects with lasers and missiles, although missiles are more powerful and thus limited based on level
 * The bad guys also automatically shoot either a laser or missile every few seconds, which user must watch out for.
 * 
 * User has a health bar and, when it is empty, he or she dies, triggering game over.
 * To prevent user from dodging everything and not fighting back, he or she must not let too many bad ships and asteroids
 * pass by them, or game over will occur.
 * Health refills every level, as does the count of enemies let past the user.
 * 
 * To win: user must complete all 6 increasingly difficult levels without death occuring.
 * As levels progress, enemies spawn at a faster rate and shoot at a faster rate
 * As levels progress, different types of Tie Fighters can spawn, and the different types get progressively more powerful
 * and take more damage from user's ballistics to kill
 * 
 * A score count and level indicator is shown in the SOUTH pane as JLabels
 * A count displaying number of available missiles left is shown in SOUTH pane as JLabel
 * A pause and unpause JButton is included in the SOUTH pane
 * --> clicking pause button triggers boolean to be true and keeps infinitely pausing the thread
 * --> clicking unpause button triggers boolean to be false, exiting the pause loop and allowing main game loop to proceed from its 
 *     exact state pre-pause
 *    
 * ArrayLists are used to store the enemy ships and asteroids that spawn, as well as the ballistics the users and enemies shoot.
 * 
 * Each time through the main (innermost) game loop, checks are made while iterating through the arraylists to determine if collisions
 * occur between various on screen objects
 * 
 * Elements found in ArrayLists that have died, left constrains of window, or otherwise need to be removed are tagged for removal
 * Iterators (the object kind used with lists) proceed through ArrayLists and remove tagged elements from it
 * --> using Iterators allows safe element removal simultaneously with iteration --> no Concurrent Modification Exception
 * --> avoids creating more ArrayLists for elements to be removed, and then removing elements of these ArrayLists from main ArrayLists
 * 
 * The only noticeable flaw is a slight lagginess in gameplay, especially when user is shooting ballistics
 * Probably due to constant usage of the remove(Object x) operation while looping through the ArrayLists to remove obsolete elements
 * remove(), unlike get() or add() for ArrayLists, has complexity O(n) and in worst case must traverse all n elements to find right one.
 * This effects performance, I suspect.
 * 
 * The class inheritance diagram is kinda ugly too - sorry:( - when I first created them I did not forsee all the connections I would later
 * need, and the whole program sorta evolved organically as I went along discovering things.
 * 
 * 
 */
...
public class StarInvaders extends GraphicsProgram
{
            ...
            while (true) {
                ...
                if (count % enemySpawnInterval == 0) {//each loop takes ~10ms --> this block executes ~every enemySpawnInterval*10 ms = 2 sec in level 1, etc.
                    ...
                    if ((kind == 0 || enemiesSpawned >= numObjectsToSpawn) && roidsSpawned < numObjectsToSpawn) {
                        ...
                        BadShip willSpawn = makeRandomTieAtRandomSpawn(level-1);
                        add(willSpawn);
    ...
    private BadShip makeTieAt (int type,double x,double y) {
        switch (type) {
            case 0:
            BadShip a = new BadShip(BadShip.tieFighterFile,x,y);
            return a;
            case 2:
            BadShip b = new BadShip(BadShip.tiePhantomFile,x,y);
            return b;
            case 1:
            BadShip c = new BadShip(BadShip.tieInterceptorFile,x,y);
            return c;
            case 3:
            BadShip d = new BadShip(BadShip.tieBomberFile,x,y);
            return d;
            case 6:
            BadShip e = new BadShip(BadShip.tieVaderFile,x,y);
            return e;
            case 5:
            BadShip f = new BadShip(BadShip.biggerBomberFile,x,y);
            return f;
            case 4:
            BadShip g = new BadShip(BadShip.biggerFighterFile,x,y);
            return g;
            default:
            return null;
    ...
    private void removeAllButBackground () {
        removeAll();
        add(background);
    }
}
